...
const Index = () => {
  ...
  const formatRemoteDays = (remoteDays: number[]): string => {
    ...
    return remoteDays.sort((a, b) => a - b) // Sort days in order
    .map(day => {
  ...
  const totalEmployees = businessUnits.reduce((total, unit) => total + unit.employees.length, 0);

  // Prepare and memoize the base employee data array
  const allEmployeesWithUnit = useCallback(() => businessUnits.flatMap(unit => unit.employees.map(employee => ({
    ...employee,
  ...
  const applyStatusFilter = useCallback(employees => {
  ...
  const applySearchFilter = useCallback(employees => {
    ...
    return employees.filter(employee => employee.name.toLowerCase().includes(query) || employee.businessUnit.toLowerCase().includes(query));
  }, [searchQuery]);
  ...
  const handleEmployeeImport = (employees: Partial<Employee>[]) => {
    ...
    Object.entries(employeesByBusinessUnit).forEach(([unitName, unitEmployees]) => {
      ...
      let businessUnit = updatedBusinessUnits.find(unit => unit.name.toLowerCase() === unitName.toLowerCase());
  ...
  const SearchInput = () => <div className="relative mb-4">
      <Search className="absolute left-2.5 top-2.5 h-4 w-4 text-gray-500" />
      <Input type="text" placeholder="Search by name or team..." defaultValue={searchQuery} onChange={handleSearchInputChange} className="pl-9 w-full md:w-64" />
    </div>;
  return <div className="min-h-screen bg-app-light p-6 animate-fade-in">
        ...
        <Card className="md:col-span-2 animate-slide-up">
          ...
          <CardContent>
            <div className="flex flex-col space-y-4">
              <Calendar mode="single" selected={selectedDate} onSelect={date => date && setSelectedDate(date)} className="rounded-md border shadow-sm" />
        ...
        <Card className="animate-slide-up [animation-delay:200ms]">
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex space-x-2">
                ...
                <Button variant="outline" size="sm" className="The button is outside viewport\n">
          ...
          <CardContent>
            <div className="space-y-4">
              {businessUnits.map(unit => <div key={unit.id} className="p-4 rounded-lg border bg-white shadow-sm hover:shadow-md transition-shadow">
          ...
          <CardContent>
            <Tabs value={employeeView} onValueChange={value => setEmployeeView(value as EmployeeView)}>
              ...
              <TabsContent value="all" className="mt-0">
                <SearchInput />
                <EmployeeTable employees={filteredEmployees} selectedDate={selectedDate} isRemoteFn={isEmployeeRemoteOnDate} formatRemoteDaysFn={formatRemoteDays} />
              </TabsContent>
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="animate-slide-up [animation-delay:600ms]">
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              Employees present on {selectedDate.toLocaleDateString('en-US', {
              weekday: 'long',
              month: 'short',
              day: 'numeric'
            })}
        ...
        <Card className="animate-slide-up [animation-delay:700ms]">
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              Employees working remotely on {selectedDate.toLocaleDateString('en-US', {
              weekday: 'long',
              month: 'short',
              day: 'numeric'
            })}
      ...
      <TableBody>
        {employees.length === 0 ? <TableRow>
            <TableCell colSpan={5} className="text-center py-8 text-gray-500">
              ...
              <TableCell>
                <Badge variant="outline" className={isRemoteFn(employee, selectedDate) ? "bg-blue-50" : "bg-green-50"}>
                  {isRemoteFn(employee, selectedDate) ? "Remote" : "In Office"}
                </Badge>
              </TableCell>
